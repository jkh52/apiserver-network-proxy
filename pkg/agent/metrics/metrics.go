...
func (a *AgentMetrics) Reset() {
	...
	a.streamEvents.Reset()
	a.streamEventsErrors.Reset()
}
...
func (a *AgentMetrics) ObservePacket(segment sharedmetrics.Segment, packetType client.PacketType) {
	sharedmetrics.ObservePacket(a.streamEvents, segment, packetType)
}

func (a *AgentMetrics) ObserveStreamErrorNoPacket(segment sharedmetrics.Segment, err error) {
	sharedmetrics.ObserveStreamErrorNoPacket(a.streamEventsErrors, segment, err)
}

func (a *AgentMetrics) ObserveStreamError(segment sharedmetrics.Segment, err error, packetType client.PacketType) {
	sharedmetrics.ObserveStreamError(a.streamEventsErrors, segment, err, packetType)
}
